...
function commitWork(fiber) {
	...
	// 获取当前fiber节点的父节点
	// 自定义的组件是没有dom内容的，所以递归向上寻找，找到第一个存在dom的节点
	let domParentFiber = fiber.parent;
	while (!domParentFiber.dom) {
		domParentFiber = domParentFiber.parent;
	}
	// 获取找到的dom，它是当前fiber节点在dom树中的父元素
...
function commitDeletion(fiber, domParent) {
	// 删除对应dom，当前节点没有dom时递归找到有dom的子节点进行删除
	if (fiber.dom) {
...
function render(element, container) {
	// 定义根节点，根节点是容器节点，根节点的子节点才是需要渲染的内容
	wipRoot = {
		dom: container,
		props: {
			children: [element],
